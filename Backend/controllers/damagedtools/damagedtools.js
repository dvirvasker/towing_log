const DamagedTools = require("../../models/damagedtools/damagedtools");

exports.find = (req, res) => {
    DamagedTools.find((err, damagedtools) => {
    if (err) res.send(err);
    res.json(damagedtools);
  }).sort({ createdAt: -1 });
};

exports.findById = (req, res) => {
    DamagedTools.findById(req.params.id, (err, damagedtools) => {
    if (err) res.send(err);
    res.json(damagedtools);
  });
};
...
exports.read = async (req, res) => {
    DamagedTools.findById(req.params.id)
...
exports.create = (req, res) => {
  ...
  const damagedtools = new DamagedTools({
    c4_trained,
    c4_treatment,
    c4_shutdown,
    c4_rescue,
...
exports.update = (req, res) => {
  ...
  DamagedTools.findById(req.params.id).then((request) => {
...
exports.remove = (req, res) => {
    DamagedTools.findByIdAndDelete(req.params.id)
    .then((damagedtools) => res.json(damagedtools))
    .catch((err) => res.status(400).json("Error: " + err));
};
