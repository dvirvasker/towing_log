const Halfim = require("../../models/halfim/halfim");

exports.find = (req, res) => {
  Halfim.findOne((err, halfim) => {
    if (err) res.send(err);
    res.json(halfim);
  }).sort({ createdAt: -1 });
};
...
exports.findById = (req, res) => {
  Halfim.findById(req.params.id, (err, halfim) => {
    if (err) res.send(err);
    res.json(halfim);
  });
};
...
exports.remove = (req, res) => {
  Halfim.findByIdAndDelete(req.params.id)
    .then((halfim) => res.json(halfim))
    .catch((err) => res.status(400).json("Error: " + err));
};

exports.findbypersonalnumber = (req, res) => {
  Halfim.find({ personalnumber: req.params.id })
    .then((halfim) => res.json(halfim))
    .catch((err) => res.status(400).json("Error: " + err));
};
