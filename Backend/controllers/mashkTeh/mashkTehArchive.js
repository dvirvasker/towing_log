const MashkTehArchive = require("../../models/mashkTeh/mashkTehArchive");

exports.find = (req, res) => {
  MashkTehArchive.findOne((err, mashkTeh) => {
    if (err) res.send(err);
    res.json(mashkTeh);
  }).sort({ createdAt: -1 });
};
...
exports.findById = (req, res) => {
  MashkTehArchive.findById(req.params.id, (err, mashkTeh) => {
    if (err) res.send(err);
    res.json(mashkTeh);
  });
};
...
exports.read = async (req, res) => {
  MashkTehArchive.findById(req.params.id)
...
exports.create = (req, res) => {
  ...
  const mashkTehArchive = new MashkTehArchive({
    personalnumber,
    hatcim,
    hatcim_kshirot,
    bkhas,
...
exports.update = (req, res) => {
  MashkTehArchive.findOne().then((request) => {
...
exports.remove = (req, res) => {
  MashkTehArchive.findByIdAndDelete(req.params.id)
