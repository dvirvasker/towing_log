const SdacPowerArchive = require("../../models/sdacPower/sdacPowerArchive");

exports.find = (req, res) => {
  SdacPowerArchive.findOne((err, sdacPowerArchive) => {
    if (err) res.send(err);
    res.json(sdacPowerArchive);
  }).sort({ createdAt: -1 });
};

exports.findById = (req, res) => {
  SdacPowerArchive.findById(req.params.id, (err, sdacPowerArchive) => {
    if (err) res.send(err);
    res.json(sdacPowerArchive);
  });
};
...
exports.read = async (req, res) => {
  SdacPowerArchive.findById(req.params.id)
...
exports.create = (req, res) => {
  ...
  const sdacPowerArchiveArchive = new SdacPowerArchive({
    personalnumber,
     darom_tank,
     darom_namer,
     darom_hashaz,
  ...
  sdacPowerArchiveArchive.save((err, data) => {
    if (err) {
...
exports.update = (req, res) => {
  SdacPowerArchive.findOne().then((request) => {
...
exports.remove = (req, res) => {
  SdacPowerArchive.findByIdAndDelete(req.params.id)
    .then((sdacPowerArchive) => res.json(sdacPowerArchive))
    .catch((err) => res.status(400).json("Error: " + err));
};
