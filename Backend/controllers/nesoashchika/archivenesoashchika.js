const ArchiveNesoaShchika = require("../../models/nesoashchika/archivenesoashchika");

exports.find = (req, res) => {
    ArchiveNesoaShchika.find((err, archivenesoashchika) => {
    if (err) res.send(err);
    res.json(archivenesoashchika);
  }).sort({ createdAt: -1 });
};

exports.findById = (req, res) => {
    ArchiveNesoaShchika.findById(req.params.id, (err, archivenesoashchika) => {
    if (err) res.send(err);
    res.json(archivenesoashchika);
  });
};
...
exports.read = async (req, res) => {
    ArchiveNesoaShchika.findById(req.params.id)
...
exports.create = (req, res) => {
  ...
  const archivenesoashchika = new ArchiveNesoaShchika({
    personalnumber, 
  ...
  archivenesoashchika.save((err, data) => {
    if (err) {
...
exports.update = (req, res) => {
  ...
  ArchiveNesoaShchika.findById(req.params.id).then((request) => {
...
exports.remove = (req, res) => {
    ArchiveNesoaShchika.findByIdAndDelete(req.params.id)
    .then((archivenesoashchika) => res.json(archivenesoashchika))
    .catch((err) => res.status(400).json("Error: " + err));
};
