...
const TemplateExample = require("../models/TemplateExample.model");
const { upload } = require("../helpers/filehelper");
const MultipleFile = require("../models/fileuploader/multipleFile");
// const referenceId = 1;

router.route("/").get((req, res) => {
  TemplateExample.find()
...
router.route("/activeRequests").get((req, res) => {
  TemplateExample.find({ status: { $lte: 100 } })
    .sort({ createdAt: -1 })
    .exec()
    .then((request) => res.json(request))
    .catch((err) => res.status(400).json("Error: " + err));
...
router.route("/archivedRequests").get((req, res) => {
  TemplateExample.find({ status: { $gte: 125 } })
    .sort({ createdAt: -1 })
    .exec()
    .then((request) => res.json(request))
    .catch((err) => res.status(400).json("Error: " + err));
...
router.route("/add").post((req, res) => {
  ...
  const newTemplateExample = new TemplateExample({
    typeRequest,
    user_card_number,
    forTypePrint,
    unit,
  ...
  const formId = newTemplateExample.save((err, form) => {
    if (err) {
...
router.route("/requestByPersonalnumber/:personalnumber").get((req, res) => {
  ...
  TemplateExample.find({ personalnumber: personalnumber })
    .then((request) => res.json(request))
    .catch((err) => res.status(400).json("Error: " + err));
});

router.route("/:id").get((req, res) => {
  TemplateExample.findById(req.params.id)
    .then((request) => res.json(request))
    .catch((err) => res.status(400).json("Error: " + err));
});

router.route("/:id").delete((req, res) => {
  TemplateExample.findByIdAndDelete(req.params.id)
    .then(() => res.json("TemplateExample deleted."))
    .catch((err) => res.status(400).json("Error: " + err));
});
...
router.route("/update/:id").post((req, res) => {
  TemplateExample.findById(req.params.id)
    .then((request) => {
      ...
      request
        .save()
        .then(() => res.json("TemplateExample updated!"))
        .catch((err) => res.status(400).json("Error: " + err));
    })
    .catch((err) => res.status(400).json("Error: " + err));
});
...
router.route("/sameRequest/:id").get((req, res) => {
  ...
  TemplateExample.findById(req.params.id)
    .then((request) => {
      ...
      TemplateExample.find({ unit: unitName, toraHeilitVolumes: dataToraHeilit })
        .then((requestData) => {
...
router.route("/statusUpdate/:id").post((req, res) => {
  ...
  TemplateExample.findById(req.params.id)
    .then((request) => {
      ...
      request
        .save()
        .then(() => res.json("TemplateExample status updated!"))
        .catch((err) => {
          // console.log(err);
